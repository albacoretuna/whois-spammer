    ...
    return new Promise((resolve, reject) => {
        whois.stdout.on('data', (data) => {
            if(data.indexOf('Domain') !== -1) {
                domainsArray.push(url);
                resolve(url);
                console.log('resolved single doWhois');
            } else {
                reject(url);
            }
        });
        ...
        whois.stderr.on('data', (data) => {
            ...
            reject(data);
            console.log('rejected single doWhois');
        });
    });
};

//const twoLetterWordsFi = twoLetterWords.map((name) => name+'.fi' );
/*
...
const askAllUrls = (allUrls) => {
	...
	Promise.all(successArray).then(() => {
	    ...
	    console.log('all resolved', domainsArray);
	})
	    .catch((err)=>(console.log(err)));
    });
};
askAllUrls(['omid.fi', 'adniiiiasdsdiasd.fi', 'basdfasdfsdssb.fi', 'asdfasasdffasdfasd.fi',
    'omid.fi',
    'adniiiiiasdsdfsdf.fi',
    'bsdfb.fi',
    'asdfasdfasddfasd.fi']).then((allUrls) => console.log(allUrls));
console.log(sitesAvailable);
// end
